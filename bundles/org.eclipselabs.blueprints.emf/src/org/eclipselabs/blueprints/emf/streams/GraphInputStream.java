...
public class GraphInputStream extends InputStream implements URIConverter.Loadable {
	...
	public GraphInputStream(Graph graph) {
		this.graph = graph;
	}
	...
	public void loadResource(Resource resource) throws IOException {
		...
		if (uri.hasFragment()) {
			...
			final Vertex resourceVertex = getResourceVertex(uri);
	...
	private EObject createEObject(Vertex vertex, Resource resource) {
		final String eClassURI = (String) vertex.getProperty(PropertyKind.eClass.toString());
	...
	private void fill(Vertex vertex, EObject eObject, EReference eReference) {
		...
		if (edges.iterator().hasNext()) {
			...
			if (eReference.isMany()) {
				...
				for (Edge edge: edges) {
					EObject eTarget = getReferenceEObject(eObject, vertex, edge);
					if (eTarget != null) {
						Integer position = (Integer) edge.getProperty("_position");
	...
	private Vertex getResourceVertex(URI resourceURI) {
